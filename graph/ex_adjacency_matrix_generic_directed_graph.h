...
#ifndef EX_ADJACENCY_MATRIX_GENERIC_DIRECTED_GRAPH_H
#define EX_ADJACENCY_MATRIX_GENERIC_DIRECTED_GRAPH_H
...
namespace Ex::AdjacencyMatrix {
    ...
    public:
        explicit GenericDirectedGraph(unsigned long vertexMaxNum)
        ...
        void remove_vertex(unsigned long num) {
            if (check_vertex(num)) {
                ...
                for (unsigned long j = 1; j < vertexMaxNum; ++j) {
                    if (vertices[num][j].weight != 0) {
                        vertices[num][j].num = std::optional<long>();
                        vertices[num][j].weight = 0;
                        vertices[num][j].data = std::optional<T>();
                        edgeNum--;
                    }
                }
    ...
    void ex_adjacency_matrix_generic_directed_graph_test();

}

#endif //EX_ADJACENCY_MATRIX_GENERIC_DIRECTED_GRAPH_H
